...
app.post('/api/analyze50', async (req, res) => {
  ...
  try {
    ...
    // CSSを収集（style タグ + inline + 外部CSS）
    let allCSS = '';
    $('style').each((_, el) => { allCSS += $(el).html() + '\n'; });
    // 外部CSSも取得を試みる
    ...
    if (err.code === 'ENOTFOUND' || err.code === 'ERR_INVALID_URL') {
      return res.status(400).json({ error: 'URLが見つかりません。正しいURLを入力してください。' });
    }
    if (err.code === 'ECONNABORTED') {
      return res.status(400).json({ error: 'タイムアウトしました。' });
    }
    return res.status(500).json({ error: `分析エラー: ${err.message}` });
  }
});
